...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({ className, albumTitle, artistName, tracks, cover }) => {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      const filteredDurations = entries.reduce((acc, [src, d]) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isActive ? 'border border-primary bg-secondary/60' : 'border-transparent hover:bg-secondary/50 hover:border-foreground/30'
                    )}
                    onClick={() => handleSelectTrack(index)}
                    role='button'
                    aria-label={`Lire ${track.title}`}
                  >
                    <div className={cn('text-xs tabular-nums w-6 text-right', isActive && 'text-primary')}>{
                      index + 1
                    }</div>
                    ...
                    <div className='flex-1 min-w-0'>
                      <div className={cn('truncate text-sm sm:text-base', isActive && 'text-primary font-medium')}>{track.title}</div>
