...
export function SeoHead({
  ...
  // fallbacks from i18n
  const title =
    customTitle ?? t(`${canonicalPath === '/' ? 'home' : canonicalPath.slice(1)}.title`);
  const description =
    customDesc ?? t(`${canonicalPath === '/' ? 'home' : canonicalPath.slice(1)}.description`);
  const keywords =
    customKeywords ?? t(`${canonicalPath === '/' ? 'home' : canonicalPath.slice(1)}.keywords`);
